...
function expandTcoUrls(text: string, urls: any[] | undefined): string {
...
function parseTweetText(tweet: any): string {
  ...
  const sourceText = String(noteText ?? legacyText ?? "");
  const urls = Array.isArray(noteResult?.entity_set?.urls)
    ? noteResult.entity_set.urls
    : tweet?.legacy?.entities?.urls;
  return expandTcoUrls(sourceText, urls).trim();
}
