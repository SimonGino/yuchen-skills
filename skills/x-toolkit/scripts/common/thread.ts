import { BOOKMARKS_RETRY_CONFIG, fetchTweetDetail } from "./graphql";
import { unwrapTweetResult } from "./tweet-utils";
import type { RetryConfig } from "./graphql";
...
export async function fetchTweetThread(
  ...
  const res = await fetchTweetDetail(tweetId, cookieMap, undefined, retryConfig);
  if (includeResponses) {
    responses.push(res);
  }
  ...
  while (topCursor && topHasThread && maxRequestCount > 0) {
    const newRes = await fetchTweetDetail(tweetId, cookieMap, topCursor, retryConfig);
    if (includeResponses) {
  ...
  async function checkMoreTweets(focalId: string) {
    while (moreCursor && hasThread && maxRequestCount > 0) {
      const newRes = await fetchTweetDetail(focalId, cookieMap, moreCursor, retryConfig);
      if (includeResponses) {
        responses.push(newRes);
      }
    ...
    if (bottomCursor) {
      const newRes = await fetchTweetDetail(focalId, cookieMap, bottomCursor, retryConfig);
      if (includeResponses) {
        responses.push(newRes);
      }
  ...
  if (lastEntity?.tweet?.legacy?.id_str) {
    const lastRes = await fetchTweetDetail(lastEntity.tweet.legacy.id_str, cookieMap, undefined, retryConfig);
    if (includeResponses) {
  ...
  const rootIndex = allEntries.findIndex((entry) => entry.tweet?.legacy?.id_str === rootEntry.id_str);
  if (rootIndex > 0) {
