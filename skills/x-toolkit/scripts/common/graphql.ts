...
export const BOOKMARKS_RETRY_CONFIG: RetryConfig = {
  maxAttempts: 5,
  delayMs: 60_000,
  backoffFactor: 2,
...
export const TWEETS_RETRY_CONFIG: RetryConfig = {
  maxAttempts: 4,
  delayMs: 15_000,
  backoffFactor: 3,
...
function parseRateLimitResetMs(response: Response): number | null {
  ...
  return resetMs > 0 ? resetMs + 2000 : null;
}

export function buildGraphqlRetryOptions(
...
async function fetchXApiJson(
  ...
  return retryWithBackoff(
    async () => {
      ...
      if (!response.ok) {
        const retryAfterMs =
          retryConfig.respectRateLimit && response.status === 429 ? parseRateLimitResetMs(response) : null;
    ...
    buildGraphqlRetryOptions(retryConfig),
  );
}
...
async function resolveTweetDetailQueryInfo(userAgent: string): Promise<ArticleQueryInfo> {
  ...
  const featureMatch = chunk.match(/operationName:\"TweetDetail\"[\s\S]*?featureSwitches:\[(.*?)\]/);
  const fieldToggleMatch = chunk.match(/operationName:\"TweetDetail\"[\s\S]*?fieldToggles:\[(.*?)\]/);
...
async function fetchTweetResult(
  ...
  return fetchXApiJson(url.toString(), buildRequestHeaders(cookieMap, userAgent, bearerToken), retryConfig);
}
...
export async function fetchTweetDetail(
  ...
  cookieMap: XCookieMap,
  cursor?: string,
  retryConfig: RetryConfig = BOOKMARKS_RETRY_CONFIG,
): Promise<unknown> {
...
export async function fetchXArticle(
  ...
  const tweetPayload = await fetchTweetResult(articleId, cookieMap, userAgent, bearerToken, retryConfig);
  if (raw) {
    return tweetPayload;
  }
  ...
  const articlePayload = await fetchArticleEntityById(articleId, cookieMap, userAgent, bearerToken, retryConfig);
...
export async function fetchXTweet(
  ...
  const tweetPayload = await fetchTweetResult(tweetId, cookieMap, userAgent, bearerToken, retryConfig);
  if (raw) {
    return tweetPayload;
  }
