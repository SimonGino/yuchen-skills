...
import { BOOKMARKS_RETRY_CONFIG } from "./graphql";
import { hasRequiredXCookies, loadXCookies } from "./cookies";
import { fetchTweetThread } from "./thread";
...
export async function tweetToMarkdown(
  ...
  const retryConfig = options.retryConfig ?? BOOKMARKS_RETRY_CONFIG;
  if (!options.cookieMap) {
    log("[tweet-to-markdown] Loading cookies...");
  }
  const cookieMap = options.cookieMap ?? (await loadXCookies(log));
  ...
  const thread = await fetchTweetThread(tweetId, cookieMap, false, retryConfig);
  if (!thread) {
  ...
  const author = username && name ? `${name} (@${username})` : username ? `@${username}` : name ?? null;
  ...
  const articleEntity = await resolveArticleEntityFromTweet(firstTweet, cookieMap, retryConfig);
...
async function main() {
  ...
  if (!url) {
    console.error("Usage:");
    console.error("  npx -y bun skills/x-toolkit/scripts/common/tweet-to-markdown.ts <tweet url>");
    process.exit(1);
  }
