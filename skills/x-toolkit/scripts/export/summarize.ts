import { resolveOpenAiApiFormat } from "../common/openai-format";
import { buildEnvWithFileOnlyKeysFromWqqSkillsEnv } from "../common/wqq-skills-env";
...
export async function summarizeMarkdownToChinese(
  ...
  const env =
    options.env ||
    (await buildEnvWithFileOnlyKeysFromWqqSkillsEnv(FILE_ONLY_ENV_KEYS, process.env, process.env.HOME));
